...
header = """# THIS IS AN AUTOGENERATED FILE, DON'T MODIFY IT.
...
def parse(lines):
    ...
    for line in lines:
        ...
        if ma:
            ...
            attrs.append(map(str.strip, ma.groups()))
        if mv:
            n = mv.group(1)
            if n in vars_to_change:
                ...
                v = re.sub('/\*.*\*/', '', mv.group(2))
            vars.append((n, str.strip(v)))
        if mm:
            methods.append(map(str.strip, mm.groups()))
